...
| [`coordinator.py`](../custom_components/renac_wallbox/coordinator.py) | `RenacWallboxCoordinator` polls `api/charging/index` (2.4) every 10s by default (configurable via `number.*_polling_interval` or Configure, 10–3600s); `RenacSettingsCoordinator` polls `api/charging/basic|fast|pv|off-peak` (2.10) every 5 minutes, best-effort (a failed group doesn't block setup) |
...
| [`number.py`](../custom_components/renac_wallbox/number.py) | 10 numeric entities: max current limit + polling interval (both realtime-backed, local-only for the latter — never sent to RENAC), protect temp, meter address, PV min. solar power, PV/off-peak/fast energy targets, fast cost target (settings-backed, 2.10) |
...
| [`dashboard.py`](../custom_components/renac_wallbox/dashboard.py) | Best-effort helper run at the end of `async_setup_entry`: resolves this device's real entity IDs (via `entity_registry`, matching the `unique_id` patterns above) into a ready-to-paste `type: grid` Lovelace card, writes it to `<config>/renac_wallbox_dashboard_<inv_sn>.yaml`, and raises a one-time persistent notification pointing at the file the first time it's created. Deliberately does not write into HA's Lovelace storage directly — see the module docstring for why. Regenerated (silently, no repeat notification) on every setup/reload so entity renames stay in sync |
...
| [`coordinator.py`](../custom_components/renac_wallbox/coordinator.py) | `RenacWallboxCoordinator` dotazuje `api/charging/index` (2.4) defaultně každých 10 s (nastavitelné přes entitu `number.*_polling_interval` nebo Konfigurovat, 10–3600 s); `RenacSettingsCoordinator` dotazuje `api/charging/basic|fast|pv|off-peak` (2.10) každých 5 minut, best-effort (selhání jedné skupiny neblokuje nastavení) |
...
| [`number.py`](../custom_components/renac_wallbox/number.py) | 10 číselných entit: limit max. proudu + interval obnovení (oba napojeny na reálný coordinator, druhý čistě lokální nastavení HA — nikdy se neposílá do RENAC), ochranná teplota, adresa elektroměru, PV min. solární výkon, cíle energie PV/nízký tarif/rychlé nabíjení, cíl nákladů rychlého nabíjení (napojeno na settings, 2.10) |
...
| [`dashboard.py`](../custom_components/renac_wallbox/dashboard.py) | Best-effort pomocník spouštěný na konci `async_setup_entry`: přes `entity_registry` (podle vzorů `unique_id` výše) dohledá skutečná entity ID tohoto zařízení, sestaví z nich hotovou kartu Lovelace (`type: grid`) a zapíše ji do `<config>/renac_wallbox_dashboard_<inv_sn>.yaml`; při prvním vytvoření souboru vyvolá jednorázové persistentní upozornění s odkazem na něj. Záměrně nezapisuje přímo do úložiště Lovelace HA — důvod je v docstringu modulu. Soubor se tiše přegeneruje (bez opakovaného upozornění) při každém nastavení/reloadu, takže zůstává v souladu s případným přejmenováním entit |
